...
export function Pagination(props: PaginationProps) {
  ...
  const toBetweenFirstAndCurrent = React.useCallback(() => {
    const page = Math.round(
      (first[first.length - 1] + current.concat(last)[0]) / 2,
    );
    setCurrentPage(page);
  }, [first, current, last]);
  const toBetweenCurrentAndLast = React.useCallback(() => {
    ...
    const page = Math.round(
      (lowerPageNumbers[lowerPageNumbers.length - 1] + last[0]) / 2,
    );
    setCurrentPage(page);
  }, [first, current, last]);
    ...
    <Wrapper className='rdt-search__pagination'>
      ...
      <PageNumbers className='pagenumbers'>
        ...
        {first.length > 0 && current.length > 0 && (
          <PaginationButton
            onClick={toBetweenFirstAndCurrent}
            color={context.style.spotColor}
          >
            …
          </PaginationButton>
        )}
        ...
        {last.length > 0 && (
          <PaginationButton
            onClick={toBetweenCurrentAndLast}
            color={context.style.spotColor}
          >
            …
          </PaginationButton>
      ...
      {props.currentPage !== pageCount ? (
        <Next onClick={toNext} color={context.style.spotColor}>
          ▸
        </Next>
      ) : (
