...
export function useSavedSearches(url: string): [SavedSearch[], (ss: SavedSearch) => void] {
	const [savedSearches, setSavedSearches] = React.useState<SavedSearch[]>([])

	const saveSearch = React.useCallback((savedSearch: SavedSearch) => {
		const nextSavedSearches = [...savedSearches]
		nextSavedSearches.unshift(savedSearch)
		localStorage.setItem(getStorageKey(url), serializeObject(nextSavedSearches))
