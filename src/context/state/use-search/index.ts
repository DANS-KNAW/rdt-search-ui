...
export function useSearch({
	...
	React.useEffect(() => {
		if (props.url == null || props.url.length === 0 || controllers.size === 0) return  // || searchState.isInitialSearch) return

		const changedProps = prev == null
			? Object.keys(state)
			: Object.keys(state).filter(key => state[key as keyof SearchState] !== prev[key as keyof SearchState])
		...
		if (
			...
			fetchSearchResultOnly(state, props, controllers)
				.then(searchResult => {
					dispatch({
		...
		// Only facet states have changed, only update the facet values of one facet
			...
			fetchFacetValuesOnly(state, props, controllers, facetController)
				.then(values => {
					dispatch({
...
async function fetchSearchResultOnly(searchState: SearchState, searchProps: SearchProps, controllers: FacetControllers) {
...
async function fetchSearchResultWithFacets(searchState: SearchState, searchProps: SearchProps, controllers: FacetControllers) {
	const { payload } = new ESRequestWithFacets(searchState, searchProps, controllers)
	const response = await fetchSearchResult(searchProps.url, payload)
