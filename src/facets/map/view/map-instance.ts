...
export const markerStyle = (feature: Feature) => {
	...
	style.getText().setText(feature.get('count') > 1 ? feature.get('count').toString() : undefined)
...
export function getMapInstance(target?: HTMLElement) {
	return new OLMap({
		...
		view: new View({
			center: [0, 0],
			zoom: 0,
			extent: transformExtent([-180, -90, 180, 90], 'EPSG:4326', 'EPSG:3857') as [number, number, number, number],
		})
	})
