...
import { HistogramFacetProps } from '.';
...
interface BarProps {
	activeArea: HistogramFacetValue
	filter: HistogramFacetValue
	row: number
}
...
export default function Bars({ facetState, facet }: HistogramFacetProps) {
	const searchContext = React.useContext(SearchStateContext)

	const setRange = React.useCallback(() => {
		console.log('TODO set range')
		// if (facetState.filter.length < 2) {
		...
		// 		type: 'SET_FILTER',
		// 		facetId: facet.ID,
		// 		value: facetState.filter[facetState.filter.length - 2] 
		// 	})
		// }
	}, [facet.ID])
	...
	return (
		<Wrapper>
			{
				Array.isArray(facetState.filter) &&
				facetState.filter.length > 0 &&
				<RemoveFilterButton
					onClick={setRange}
					title="Remove filter"
				>
			...
			<svg viewBox={`0 0 ${BAR_WIDTH} ${((facetState.filter || []).length + 1) * BAR_HEIGHT}`}>
				<defs>
				...
				{
					facetState.filter.map((filter, index) => 
						<Bar
							activeArea={facetState.filter}
							key={index}
							filter={filter}
							row={-1 * index + facetState.filter!.length} /* with length 3: index 0 => row 3, index 1 => row 2, index 2 => row 1, the end */
						/>
					)
				}
				{
					Array.isArray(facetState.filter) &&
					<Bar
						row={facetState.filter.length + 1}
						filter={facetState.value}
						activeArea={facetState.filter[0]}
					/>
				}
			</svg>
		</Wrapper>
